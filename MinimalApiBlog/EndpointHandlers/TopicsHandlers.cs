...
{
    public static async Task<Ok<IEnumerable<TopicDto>>> GetTopicsAsync (BlogDbContext blogDbContext, IMapper mapper)
    {
        return TypedResults.Ok(mapper.Map<IEnumerable<TopicDto>>(await blogDbContext.Topics.ToListAsync()));
    }
        
    public static async Task<Results<NotFound, Ok<TopicDto>>> GetTopicAsync(Guid topicId, BlogDbContext blogDbContext, IMapper mapper)
    {
    ...
    public static async Task<Ok<TopicDto>> PostArticleAsync(BlogDbContext blogDbContext, IMapper mapper, TopicCreationDto topic)
    {
